...
#define MVERSION "vlogger 2.2 - by rd@thc.org\n"
...
#ifdef LINUX26
#define PGRP(task) task->__pgrp
#else
#define PGRP(task) task->pgrp
#endif
...
#define DRV_TYPE(tty) tty->driver->type
#define DRV_SUBTYPE(tty) tty->driver->subtype
#define DRV_OPEN(tty) tty->driver->open
...
#define BEGIN_FS_ROOT { int saved_fsuid = current->fsuid; current->fsuid = 0;
#define END_FS_ROOT current->fsuid = saved_fsuid; }
